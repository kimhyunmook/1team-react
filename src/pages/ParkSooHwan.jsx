        ...
        <p className="intro-text">
          {showMore && (
            <div className="intro-text-div">
              <div className="intro-text-div-div">안녕하세요. 1팀 박수환입니다.<br /> 다른 IT쪽에서 일하다가 개발일을 하고싶어 교육에 지원했습니다.</div>
              <div>처음 시작할땐 html/css도 잘 몰라서 힘들었는데, 공부하면서 몰랐던걸 배우고 배운걸로 웹사이트를 직접 만들면서 보람을 느끼는것같습니다.</div>
              <div>다들 착하신것같고, 서로 협업하면서 초급프로젝트까지 잘 마무리 되었으면 좋겠습니다.</div>
            </div>
          )}
        </p>
      ...
      <div className="id-box">
        ...
        <div className="input-box">
          <input
            value={tempId}
            onChange={onChangeId}
            className='all-item-searchBox1' placeholder='아이디 입력'></input>
          <input
            value={tempTagLine}
            onChange={onChangeTag}
            onKeyDown={onKeyDown}
            className='all-item-searchBox2' placeholder="태그(#뺴고 입력)"></input>
        </div>
        <button className="toggle-btn" onClick={onClickSearch}>조회하기</button>
        {showMore1 && (
          <div>
            <div>닉네임 : {apiAccountData.gameName}#{apiAccountData.tagLine}</div>
            <div>Level : {summonerData.summonerLevel}</div>
            <div>tier : {LeagueV4.tier}</div>
            <div>승:{LeagueV4.wins} 패:{LeagueV4.losses}</div>
            <div>승률 : {Math.round((LeagueV4.wins / (LeagueV4.wins + LeagueV4.losses)) * 100)}%</div>
            <div>
                ...
                <ul className="champion-Data">
                  {championMasteryData.slice(0, 3).map((champion, index) => (
                    <div key={champion.championId}>
                      ...
                      <div>최근 플레이: {new Date(champion.lastPlayTime).toLocaleString()}</div>
                    </div>
                  ))}
                </ul>
              )}
